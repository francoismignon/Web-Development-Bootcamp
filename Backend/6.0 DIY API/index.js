...
app.get("/filter", (req, res)=>{
  ...
  //filtrer le tableau des blagues
  const jokesMatch = jokes.filter(joke => joke.jokeType === typeOfJoke);
  res.json(jokesMatch);
});
...
app.put("/jokes/:id", (req, res) => {
  ...
  jokes[index] = jokeToModify;
  res.json(jokeToModify);
...
app.delete("/jokes/:id", (req, res)=>{
  ...
  if (index > -1) {
    ...
    res.status(404).json(
      { 
        error: `la blague avec l'id: ${id} n'as pas été trouvée, aucune blagues n'as pu être supprimée.`
      });
  }
});
